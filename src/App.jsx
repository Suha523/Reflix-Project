...
class App extends Component {
  ...
  isHaveRentedTrue = () => {
    let movies = [...this.state.moviesCatalog];
    let hasRentedTrue = movies.find((m) => m.isRented === true);
    if (hasRentedTrue !== undefined) {
      return true;
    }
    return false;
  };

  changeRented = (movieId, isAllowed) => {
    let movies = [...this.state.moviesCatalog];
    movies[movieId].isRented = !movies[movieId].isRented;
    this.setState({ moviesCatalog: movies });
  };
        ...
        <div className="App">
          ...
          <div>
            ...
            <Route
              ...
              render={() => (
                <Catalog
                  moviesCatalog={this.state.moviesCatalog}
                  isHaveRentedTrue={this.isHaveRentedTrue}
            ...
            <Route
              ...
              path="/movies/:movieId"
              render={({ match }) => (
                <MovieDetail match={match} state={this.state} />
              )}
            />
          </div>
        </div>
